...
class ResUNet_TAMW_Decoder(nn.Module):
    ...
    def forward(self, encoder_outputs):
        # edger_outputs = encoder_outputs[1]
...
if __name__ == "__main__":
    # 测试模型
    model = ResUNet_TAMW_Decoder(patch_size=torch.tensor([128,128,64]), n_classes=2)
    print(model)
    x = torch.randn(1, 8, 128, 128, 64)
    y = torch.randn(1, 8, 128, 128, 64)
    out = model(x, y)
    print(out.shape)