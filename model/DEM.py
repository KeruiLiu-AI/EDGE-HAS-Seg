...
class DEAB(nn.Module):
    def __init__(self, d_model):
        super().__init__()
        # 特征提取分支
        self.proj_1_pet = nn.Conv3d(d_model, d_model, 1)
        self.proj_1_ct = nn.Conv3d(d_model, d_model, 1)
        self.spatial_gating_unit_pet = LKA3d(d_model)
        self.spatial_gating_unit_ct = LKA3d(d_model)

        self.activation = nn.GELU()
        # 特征融合分支
        self.proj_fusion = nn.Conv3d(2 * d_model, d_model, 1)
        self.proj_2_pet = nn.Conv3d(d_model, d_model, 1)
        self.proj_2_ct = nn.Conv3d(d_model, d_model, 1)
    def forward(self, pet, ct):
        shortcut_pet = pet.clone()
        shortcut_ct = ct.clone()
        # 提取 PET 和 CT 的特征
        pet_features = self.proj_1_pet(pet)
        ct_features = self.proj_1_ct(ct)
        pet_features = self.activation(pet_features)
        ct_features = self.activation(ct_features)
        # 应用 LKA3d 注意力机制
        pet_attn = self.spatial_gating_unit_pet(pet_features)
        ct_attn = self.spatial_gating_unit_ct(ct_features)
        # 融合特征
        fused_features = torch.cat([pet_attn, ct_attn], dim=1)
        fused_features = self.proj_fusion(fused_features)
        # 应用激活函数和投影
        pet_out = self.proj_2_pet(fused_features)
        ct_out = self.proj_2_ct(fused_features)
        # 返回最终输出
        pet_out = pet_out + shortcut_pet
        ct_out = ct_out + shortcut_ct
        return pet_out, ct_out


if __name__ == '__main__':
    block = DEAB(32).cuda(0)
