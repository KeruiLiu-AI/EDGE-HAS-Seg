...
class MCSA(nn.Module):
    ...
    def forward(self, ct, pet):
        ...
        # print(ct_attention1.shape)
        # print(ct.shape)
        # ct_attention1_upscaled = self.upscale(ct_attention1)
        # # 将 ct_attention1_upscaled 调整为与 ct 相同的形状
        # ct_attention1_upscaled = torch.nn.functional.interpolate(ct_attention1_upscaled, size=(77, 76, 126),
        #                                                          mode='trilinear', align_corners=True)
        # print("ct_attention1_upscaled shape:", ct_attention1_upscaled.shape)
        # pet_attention1_upscaled = self.upscale(pet_attention1)
        # # 将 ct_attention1_upscaled 调整为与 ct 相同的形状
        # pet_attention1_upscaled = torch.nn.functional.interpolate(pet_attention1_upscaled, size=(77, 76, 126),
        #                                                          mode='trilinear', align_corners=True)
...
class ChannelAttention3D(nn.Module):
    ...
    def forward(self, x):
        b, c, h, w, d = x.size()
        y = torch.mean(x, dim=[2, 3, 4])  # 在深度、高度、宽度维度上取平均
        y = self.fc1(y)  # y 的形状应该是 (b, channels)
        y = y.relu()
        y = self.fc2(y).sigmoid().view(b, c, 1, 1, 1)
