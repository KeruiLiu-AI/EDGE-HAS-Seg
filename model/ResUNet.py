...
class ResUNet_Encoder(nn.Module):
    ...
    def __init__(self, 
        ...
        self.conv1_ct_edge = ResidualConv_Firstlayer(in_channels, num_channels[0], 3, 1, 1, self.dropout, self.conv_bias)
        self.conv2_ct_edge = ResidualConv(num_channels[0], num_channels[1], 3, 2, 1, self.dropout, self.conv_bias)
        self.conv3_ct_edge = ResidualConv(num_channels[1], num_channels[2], 3, 2, 1, self.dropout, self.conv_bias)
        self.conv4_ct_edge = ResidualConv(num_channels[2], num_channels[3], 3, 2, 1, self.dropout, self.conv_bias)
        self.conv5_ct_edge = ResidualConv(num_channels[3], num_channels[4], 3, 2, 1, self.dropout, self.conv_bias)
        
        self.conv1_pet_edge = ResidualConv_Firstlayer(in_channels, num_channels[0], 3, 1, 1, self.dropout, self.conv_bias)
        self.conv2_pet_edge = ResidualConv(num_channels[0], num_channels[1], 3, 2, 1, self.dropout, self.conv_bias)
        self.conv3_pet_edge = ResidualConv(num_channels[1], num_channels[2], 3, 2, 1, self.dropout, self.conv_bias)
        self.conv4_pet_edge = ResidualConv(num_channels[2], num_channels[3], 3, 2, 1, self.dropout, self.conv_bias)
        self.conv5_pet_edge = ResidualConv(num_channels[3], num_channels[4], 3, 2, 1, self.dropout, self.conv_bias)
        #
        # self.ca2 = ChannelAttention3D(num_channels[1])
        # self.ca3 = ChannelAttention3D(num_channels[2])
        # self.ca4 = ChannelAttention3D(num_channels[3])
        # self.ca5 = ChannelAttention3D(num_channels[4])
