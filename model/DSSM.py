...
class DAG(nn.Module):
    ...
    def forward(self, x, guidance):
        x = x.to(self.x_kernel_diff.device)
        ...
        guidance_diff = F.conv3d(input=guidance, weight=self.guidance_kernel_diff, bias=self.conv.bias,
                                 stride=self.conv.stride, padding=1, groups=self.in_channels)
...
class DAGB(nn.Module):
    def __init__(self, in_channel=3, guidance_channels=2):
        super(DAGB, self).__init__()
        self.dag1 = DAG(in_channel, guidance_channels)
        self.relu = nn.ReLU(inplace=True)
        self.bn = nn.BatchNorm3d(in_channel)

    def forward(self, feature, guidance):
        boundary_enhanced = self.dag1(feature, guidance)
...
if __name__=='__main__':
    model = SDM(8,8)
    ct=torch.randn((1,8,128,128,64))
    pet = torch.randn((1, 8, 128, 128, 64))
    y= model(ct,pet)
    print(y.shape)